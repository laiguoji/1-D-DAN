...
def handle_data2fft_form(Train_x, Test_x=None,Train_y = None, Test_y = None,Fold=5,Test_size_split=4,step_input_size = 153,NFFT = 51,noverlap = 12, stft_form = True):
...
def handle_data2fft_form_complex(Train_x, Test_x=None,Train_y = None, Test_y = None,Fold=5,Test_size_split=4,step_input_size = 153,NFFT = 51,noverlap = 12, stft_form = True):
    if Test_x is not None:
        A_X = vstack((Train_x, Test_x))
        A_Y = vstack((Train_y, Test_y))
    else :
    ...
    if stft_form:
        for i in range(step_input_size):  # 153
            # for j in range(self._bunch_size):
            in_put_1 = np.reshape(new_A_X[:, i * (NFFT - noverlap):NFFT + i * (NFFT - noverlap), :],
                                  [new_all_size, NFFT, -1, 2])
            try:
                in_put_2 = np.concatenate([in_put_2, in_put_1], axis=2)
            except:
                in_put_2 = in_put_1
        in_put_2 = np.transpose(in_put_2, [0, 2, 1, 3])
        new_A_X = in_put_2
    if Test_x is not None:
        print('data change to fft form succeed')
    return new_A_X,new_A_Y
...
def set_label(All_List_Path=' '):
    f = open(All_List_Path, 'r')
    lines = f.readlines()
    f.close()

    f = open(All_List_Path, 'w')
    f.write(lines[0])
    lines = lines[1:]
    for i in range(len(lines)):
        path_name_label = lines[i].strip().split()
        act_type = path_name_label[0].split('/')[3]
        label = ' '
        if act_type == 'walk_boxing':
            label = '4'
        elif act_type == 'run':
            label = '5'
        elif act_type == 'boxing':
            label = '2'
        elif act_type == 'sit':
            label = '6'
        elif act_type == 'walk_hold':
            label = '0'
        elif act_type == 'walk':
            label = '3'
        elif act_type == 'crawl':
            ...
            print('wrong')
        f.write(path_name_label[0]+' '+path_name_label[1]+' '+label+'\n')
    f.close()
    return
...
def generate_radar_bacth_complex(Path,colum = 6000,nb_classes = 7,NFFT = 51,noverlap = 12,frequce = 2000,
                         sub_freq = 2000,pad_to = 299,shuffle=False,stft_form = True,stft_handle = False,conv3D = False,only_real = False):
    '''
    according to the data list(txt file) generate data batch
    :param Path: data list path
    ...
    for l in range(N):
        ...
        Data_I = data['d_I']
        Data_Q = data['d_Q']
        dat_I = Data_I[0, 0::int(frequce / sub_freq)]
        dat_Q = Data_Q[0, 0::int(frequce / sub_freq)]
        dat = np.transpose(np.vstack([dat_I, dat_Q]))
        ALL_X[l] = dat
        ALL_Y[l, np.int8(label)] = 1
    if stft_form:
        ALL_X, ALL_Y = handle_data2fft_form_complex(Train_x=ALL_X, Test_x=None, Train_y=ALL_Y, Test_y=None,
                                            Fold=1, Test_size_split=1, step_input_size=step_input_size, NFFT=NFFT,
                                            noverlap=noverlap)
    if stft_handle:
        New_X_batch = np.zeros((N, pad_to, step_input_size))
        for i in range(N):
            c_n = []
            for ii in range(colum):
                c_n.append(complex(ALL_X[i,ii,0], ALL_X[i,ii,1]))
            c_n = np.array(c_n)
            freqs_1, t_1, spec_1 = signal.spectrogram(c_n, fs=sub_freq, window=('rectangular'), nperseg=NFFT,
                                                      noverlap=noverlap,
                                                      nfft=pad_to,
                                                      detrend=False, return_onesided=False,  # 'constant'
                                                      scaling='spectrum', axis=-1, mode='complex')  # 'density'
            if only_real:
                spec_1 = 10 * np.log10(abs(spec_1.real) + np.spacing(1))
            else:
                spec_1 = 10 * np.log10(abs(spec_1) + np.spacing(1))
            spec_1 = np.fft.fftshift(spec_1, 0)
            New_X_batch[i] = spec_1
        ALL_X = New_X_batch
